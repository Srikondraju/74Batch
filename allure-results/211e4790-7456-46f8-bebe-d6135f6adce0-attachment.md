  ...
  - iframe [active] [ref=e311]:
    - generic [ref=f1e8]:
      ...
      - generic [ref=f1e11]:
        ...
        - textbox "Search For:" [ref=f1e15]: "10040"
          ...
          - row "Employee Id Employee Name Job Title Employment status Sub-Division Supervisor" [ref=f1e25]:
            ...
            - cell "Supervisor" [ref=f1e38]:
              - link "Supervisor" [ref=f1e39] [cursor=pointer]:
                - /url: "#"
        - rowgroup [ref=f1e40]:
          - row "10040 Suresh Academy - - - -" [ref=f1e41]:
            - cell [ref=f1e42]:
              - checkbox [ref=f1e43]
            - cell "10040" [ref=f1e44]
            - cell "Suresh Academy" [ref=f1e45]:
              - link "Suresh Academy" [ref=f1e46] [cursor=pointer]:
                - /url: ./CentralController.php?menu_no_top=hr&id=10040&capturemode=updatemode&reqcode=EMP&currentPage=1
            - cell "-" [ref=f1e47]
            - cell "-" [ref=f1e48]
            - cell "-" [ref=f1e49]
            - cell "-" [ref=f1e50]
...
```ts
  ...
  32 |     }
  33 |     async waitStatement(){
  34 |     await this.page.waitForTimeout(3000); // Wait for 3 seconds
  35 | }
  36 | async deleteEmployeeInfo(){
  37 |     const frame=this.page.frameLocator(this.empInfo_frame);
  38 |     await frame.locator(this.dropdown_searchBy).selectOption(this.searchBy);
  39 |     await frame.locator(this.textbox_searchFor).fill(this.searchfor);
  40 |     await frame.locator(this.button_search).click();
> 41 |     await frame.locator(this.checkbox_info).check();
     |                                             ^ Error: locator.check: Error: strict mode violation: locator('//input[@name=\'chkLocID[]\']') resolved to 50 elements:
  42 |     await frame.locator(this.button_delete).click();
  43 |     console.log("Employee details deleted successfully");
  44 | 
  45 | }
  46 | async waitStatement2(){
  47 |     await this.page.waitForTimeout(3000); // Wait for 3 seconds
  48 | }
  49 | }
```